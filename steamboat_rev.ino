...
void IRAM_ATTR onTimer() {
  ...
  int32_t steam = (noise * (10 + steamLevel * 25)) / 255;
...
void loop() {
  ...
  float propLoad = pow(currentSpeed, 2.3);

  float rate;

  if (direction != engineDir && engineResponse > 0.1)
    rate = 0.004;
  else if (engineResponse < commandSpeed)
    rate = 0.01 * (1.0 - propLoad);
  else
    rate = 0.02;

  engineResponse += ((commandSpeed + steamBoost * 0.1) - engineResponse) * rate;
  ...
  // -------- KALIBRACJA (1.9s / 4 obroty) --------
  // TO JEST NAJWAŻNIEJSZA CZĘŚĆ
  float basePitch = currentSpeed;

  // lekkie rozszerzenie zakresu pracy
  basePitch = 0.18 + basePitch * 1.7;

  // mikro niestabilność (realizm parowca)
  basePitch *= (1.0 + (random(-5, 6) / 100.0));

  basePitch = constrain(basePitch, 0.18, 2.3);

  float pitchF = (engineDir >= 0) ? basePitch : basePitch * 0.82;

  voices[0].pitch = pitchF * FP_ONE;
  voices[0].volume = 80 + currentSpeed * 140 + steamBoost * 60;
  ...
  if (coalLoopsLeft == 0) {
    ...
    if (!voices[2].active) {
      ...
      if (coalLoopsLeft > 0) {
        ...
        nextCoalDelay = 40000 + random(40000);
      }
    }
  }
}
